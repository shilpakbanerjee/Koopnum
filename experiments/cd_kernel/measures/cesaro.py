"""
Cesàro / Fejér weak reconstruction of a measure on the unit circle.

This module implements the Section 4.2.1 reconstruction from
Korda–Putinar–Mezić:

    moments -> partial Fourier sums S_n
            -> Cesàro / Fejér density rho_N^{CS}
            -> cumulative distribution F_N^{CS}

Mathematically:
    S_n(theta) = sum_{k=-n}^n m_{-k} e^{i k theta}
    rho_N^{CS}(theta) = (1 / (N + 1)) sum_{n=0}^N S_n(theta)

Equivalently, rho_N^{CS} is the Fejér-smoothed Fourier reconstruction.

This gives an absolutely continuous approximating measure that converges
weakly even when the target measure has singular components.
"""
...
def fourier_partial_sum(
    ...
    """
    Compute the n-th partial Fourier sum

        S_n(theta) = sum_{k=-n}^n m_{-k} e^{i k theta}

    using the Hermitian relation m_{-k} = conjugate(m_k) for positive measures.
    """
    moments = np.asarray(moments, dtype=np.complex128)
    ...
    out = np.full_like(angles, fill_value=np.real(moments[0]), dtype=np.complex128)
...
def cesaro_density_from_moments(
    ...
    clip_negative:
        Numerical safeguard: clip small negative real parts to zero.
    normalize_mass:
        If True, renormalize the density to total mass Re(m_0).
    ...
    # In exact arithmetic rho is real-valued.
    rho = np.real(rho)
...
def cumulative_distribution_from_density(
    density: Array,
    angles: Array,
) -> Array:
    """
    Build a right-continuous discrete CDF on the angular grid from a density.
    """
